...
function setupGeometry(geom) {
    ...
    gl.bufferData(gl.ARRAY_BUFFER, positions, gl.DYNAMIC_DRAW);  // Use DYNAMIC_DRAW

    // Enable position attribute at location 0
    gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(0);

    // Setup color buffer for triangles
    ...
    // Setup color buffer for the outline
...
let jitteredPositions; // Array to hold modified positions for jittering
let positionBuffer; // store the changing data result in a global variable
...
window.addEventListener('load', async () => {
    ...
    jitteredPositions = new Float32Array(geomData.position.flat());
