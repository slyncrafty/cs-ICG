let startTime = Date.now();  // Keep track of when the program starts
...
function draw(milliseconds) {
    ...
    gl.uniform1f(program.uniforms.seconds, elapsedSeconds)
...
window.addEventListener('load', async () => {
    ...
    if (!program) return;
    program.secondsUniformLocation = gl.getUniformLocation(program, 'seconds');
    gl.useProgram(program);
